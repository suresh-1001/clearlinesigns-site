...
- Sound like a local sign expert, not a bot`;
  ...
  let _inside      = false;   // mousedown flag for outside-click detection
  ...
  function showChips() {
    ...
    CHIPS.forEach(function(text) {
      ...
      c.className = 'cch';
      c.textContent = text;
      // KEY FIX: stopPropagation prevents outside-click handler from firing
  ...
  /* ── Events ──
     KEY FIX: `#cls-btn * { pointer-events:none }` in CSS means all clicks
     on the button's children (SVG) bubble up to the button itself.
     KEY FIX: mousedown flag catches clicks inside panel BEFORE any DOM
     manipulation happens, so outside-click detection is always accurate.  */
  ...
  // mousedown fires before click, before chip clears its DOM node
